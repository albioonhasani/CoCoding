...
export function groqDevApi(): Plugin { return { name: 'codepath-ai-dev-api', configureServer(server) { server.middlewares.use(async (request: IncomingMessage, response: ServerResponse, next) => { const handler = routes[request.url?.split('?')[0] ?? '']; if (!handler) return next(); if (request.method !== 'POST') { response.writeHead(405, { Allow: 'POST' }); response.end(); return } if (!request.headers['content-type']?.includes('application/json')) { send(response, { status: 415, body: { error: 'Content-Type must be application/json.' } }); return } try { send(response, await handler(await readJson(request), clientId(request))) } catch (error) { if (error instanceof RequestBodyError) { send(response, { status: error.status, body: { error: error.message } }); return } send(response, { status: 500, body: { error: 'The AI service is temporarily unavailable.' } }) } }) } } }
